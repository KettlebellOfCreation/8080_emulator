...
int Disassembler8080(const uint8_t *code_buffer, const int pc)
{
    const uint8_t *code = &code_buffer[pc];
    int op_bytes = 1;
    ...
    default:
        std::cerr << "ERROR!! CANT FIND OPTYPE CODE 0x" << std::hex << static_cast<uint16_t>(*code) << '\n';
...
int DisassembleFile(const char *file_name)
{
    ...
    const auto buffer = static_cast<unsigned char *>(malloc(fsize));
